...
--defaults::
    equals -cdnl (cpu, disk, net, load), the plugins dool uses if none are specified

--all-plugins::
    enable every internal and external plugin dool can find

-a, --all::
    equals -cdngmypl (cpu, disk, net, page, mem, sys, proc, load)
...
--pidfile file::
    write dool's PID to file, removed automatically on exit

--debug::
    print extra diagnostic info to stderr; repeat for more detail

--devel::
    write a detailed timing log to a per-user file in the system
    temp directory, useful when debugging dool itself

-h, --help::
    show usage and a list of options, then exit

-V, --version::
    show the dool version and platform info, then exit
